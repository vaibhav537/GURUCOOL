          ...
          </tr>
{
      ...
      .map((val, index) => {
        return (
          ...
          <td className="p-3 w-[10rem] text-center select-none " key={val._id}>{index + 1}.</td>
          <td className="p-3 w-[10rem] text-center select-none " key={val.email}>{val.name}</td>
          <td className="p-3 w-[10rem] text-center select-none " key={val.password}>{val.email}</td>
          <td className="p-3 w-[10rem] text-center select-none " key={val.createdAt}>{val.category}</td>
          <td className="p-3 w-[10rem] text-center cursor-pointer hover:text-red-500 transition-all duration-300 hover:text-xl" key={val.updatedAt}><i className="fa-solid fa-trash"></i></td>
        </tr>
        );
      }) 
}
